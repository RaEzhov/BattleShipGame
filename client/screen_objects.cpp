...
Pages::Pages(sf::Vector2<float> position_, sf::Vector2<float> scale_, std::shared_ptr<sf::RenderWindow> window_):
        ScreenObject(window_), position(position_), scale(scale_) {
    previous = std::make_unique<Button>(position.x, position.y, scale_, [this]{previousPage();}, window, "<", 20, beige,
                                        "Upheavtt.ttf", "squareButton1.png", "squareButton2.png");
    next = std::make_unique<Button>(position.x, position.y + scale_.y * 100, scale_, [this]{nextPage();}, window, ">", 20, beige,
                                    "Upheavtt.ttf", "squareButton1.png", "squareButton2.png");
    background = std::make_unique<Picture>("filename", position, scale_, window);
}
...
void Pages::addTitle(const std::string &string) {
    words.emplace_back(string, position, scale, window);
}
