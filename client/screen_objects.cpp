...
Entry::Entry(sf::Vector2<float> position, sf::Vector2<float> scale, unsigned int size, std::shared_ptr<sf::RenderWindow> window_, unsigned int fontSize = 24,
...
Title::Title(const std::string& text_, sf::Vector2<float> position, sf::Vector2<float> scale_, std::shared_ptr<sf::RenderWindow> window_,int size,
...
TitleRef::TitleRef(const std::string &text_, sf::Vector2<float> position, sf::Vector2<float> scale_, std::shared_ptr<sf::RenderWindow> window_,
                   int size, std::function<void()> func, sf::Color color_, const std::string &font_):
                   Title(text_, position, scale_, std::move(window_),size, color_, font_), function(std::move(func)) {}

void TitleRef::eventCheck(sf::Event &event) {
    if (text.getGlobalBounds().contains(static_cast<float>(sf::Mouse::getPosition(*window).x),
...
Animation::Animation(sf::Vector2<float> position_, sf::Vector2<float> scale_, unsigned int frames, float speed_, const std::string& filename,
                     std::shared_ptr<sf::RenderWindow> window_): ScreenObject(window_), speed(speed_), isPlaying(false) {
    for (unsigned int i = 1; i <= frames; i++) {
        textures.emplace_back();
        (--(textures.end()))->loadFromFile(Config::instance().resources + std::to_string(i) + filename);
...
Pages::Pages(sf::Vector2<float> position_, sf::Vector2<float> scale_, std::shared_ptr<sf::RenderWindow> window_):
        ScreenObject(window_), position(position_), scale(scale_), lastPosNum(0) {
    background = std::make_unique<Animation>(position, scale * 8.f, 5, 10.f, "rotating.png", window);

    auto temp = std::make_unique<Button>(position, scale, nullptr, window, "", 20, beige,
                                         "Upheavtt.ttf", "squareButton1.png", "squareButton2.png");  // to take size
    next = std::make_unique<Button>(sf::Vector2<float>{position.x + background->getSize().width + 10.f,
                                                       position.y + background->getSize().height * 0.5f - temp->getSize().height / 2.f},
                                    scale, [this] { nextPage(); }, window, ">", 20, beige,
                                    "Upheavtt.ttf", "squareButton1.png", "squareButton2.png");
    previous = std::make_unique<Button>(sf::Vector2<float>{position.x - 10.f - temp->getSize().width,
                                                           position.y + background->getSize().height * 0.5f - temp->getSize().height / 2.f},
...
void Pages::eventCheck(sf::Event &event) {
...
NotificationPool::NotificationPool(sf::Vector2<float> scale_, std::shared_ptr<sf::RenderWindow> window_) :
        ScreenObject(window_), scale(scale_), notifications(), nextId(0) {}

void NotificationPool::eventCheck(sf::Event &event) {
...
void NotificationPool::addNotification(const std::string &info) {
    ...
    if (notifications.empty()) {
        notifications.emplace_back(info, id, sf::Vector2{10.f, 10.f}, scale, window, [this, id] { deleteNotification(id); });
        return;
    }
    auto position = (--(notifications.end()))->getSize();
    position.top += position.height + 10.f;
    notifications.emplace_back(info, id, sf::Vector2{position.left, position.top}, scale, window, [this, id] { deleteNotification(id); });
}
