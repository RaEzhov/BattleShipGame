...
DragNDropShp::DragNDropShp(const sf::Vector2<float> &scale_,
                           const sf::VideoMode &screen,
                           std::shared_ptr<sf::RenderWindow> window_)
    : ScreenObject(std::move(window_)), scale(scale_), dragged({false}) {
  float nextYPosition = 0;
  for (char i = 1; i <= 4; i++) {
    tShip[i].loadFromFile(Config::instance().resources + "Ship"
                              + std::to_string(i) + "_a.png");
    sShip[i].setTexture(tShip[i]);
    sShip[i].setScale(scale);
    startPos[i] = sf::Vector2<float>(static_cast<float>(screen.width) * 0.5f
                                         - sShip[i].getGlobalBounds().width
                                             / 2.0f - 50.f,
                                     static_cast<float>(screen.height) * 0.15f
                                         + nextYPosition);
...
void DragNDropShp::draw() const {
  for (int i = 1; i <= 4; i++) {
    window->draw(sShip[i]);
  }
}

void DragNDropShp::eventCheck(const sf::Event &event) {
...
GameField::GameField(sf::Vector2<float> position_,
                     sf::Vector2<float> scale_,
                     GameFieldState state_,
                     std::shared_ptr<sf::RenderWindow> window_,
                     std::function<void()> changeSide_) :
    ScreenObject(window_),
    field("Field1.bmp", position_, scale_, window_),
    changeSide(std::move(changeSide_)),
    border("fieldLetters.png", sf::Vector2<float>(position_.x - 32 * scale_.x,
                              position_.y - 32 * scale_.y), scale_, window_),
  ...
  for (auto &item : cells) {
    ...
    for (int j = 0; j < 10; j++) {
      ...
      item[j].setPosition({position.x + static_cast<float>(rectSize * row + row)
          * scale.x, position.y + static_cast<float>(rectSize * col + col)
          * scale.y});
      col++;
    }
    row++;
...
void GameField::eventCheck(const sf::Event &event) {
  switch (state) {
    case PLACEMENT:
      clearAvailability();
      for (auto &s : ship1) {
        if (s.eventCheck(event, &cells,
                         [this](Ship<1> *ship) { updateAvailability(ship); },
                         [this](std::pair<char, char> coords, char shipSize)
                         {removeShip(coords, shipSize);}) == REMOVED) {
          return;
        }
        s.updateAvailability(&cells);
      }
      ...
      for (auto &s : ship2) {
        if (s.eventCheck(event, &cells,
                         [this](Ship<2> *ship) { updateAvailability(ship); },
                         [this](std::pair<char, char> coords, char shipSize)
                                {removeShip(coords, shipSize);}) == REMOVED) {
          return;
        }
      ...
      for (auto &s : ship3) {
        if (s.eventCheck(event, &cells,
                         [this](Ship<3> *ship) { updateAvailability(ship); },
                         [this](std::pair<char, char> coords, char shipSize)
                         {removeShip(coords, shipSize);}) == REMOVED) {
          return;
        }
      ...
      for (auto &s : ship4) {
        if (s.eventCheck(event, &cells,
                         [this](Ship<4> *ship){ updateAvailability(ship); },
                         [this](std::pair<char, char> coords, char shipSize)
                         {removeShip(coords, shipSize);}) == REMOVED) {
          return;
        }
...

      {
        ...
        if (field.getSize().contains(mouse.x, mouse.y)) {
          ...
          unsigned char
              posI = std::floor(((mouse.x - field.getSize().left) / w) * 10),
              posJ = std::floor(((mouse.y - field.getSize().top) / h) * 10);
          for (int i = -2; i < 3; i++) {
            for (int j = -2; j < 5; j++) {
              if (posI + i > -1 && posI + i < 10
                  && posJ + j > -1 && posJ + j < 10) {
                cells[posI + i][posJ + j].eventCheck(event, state, this,
                                                     posI + i, posJ + j, scale);
              }
            }
          }
...
bool GameField::addShip(char i, char j, char shipType) {
  if (i >= 0 && i <= 9 && j >= 0 && j <= 9 && cells[i][j].isAvailable()) {
    auto newPosition = sf::Vector2<float>(
        position.x + (33.f * static_cast<float>(i) + 16.f) * scale.x,
    ...
    switch (shipType) {
      case 1:
        ...
        break;
      case 2:
        if (j <= 8 && cells[i][j].isAvailable()
            && cells[i][j + 1].isAvailable()) {
        ...
        break;
      case 3:
        if (j <= 7 && cells[i][j].isAvailable()
            && cells[i][j + 1].isAvailable() && cells[i][j + 2].isAvailable()) {
        ...
        break;
      case 4:
        if (j <= 6 && cells[i][j].isAvailable() && cells[i][j + 1].isAvailable()
            && cells[i][j + 2].isAvailable() && cells[i][j + 3].isAvailable()) {
          ship4.emplace_back(scale, std::pair{i, j}, newPosition, window);
        } else {
          return false;
        }
        break;
      default:std::cerr << "Wrong ship type!\n";
        break;
    }
    return true;
  }
  ...
  switch (shipType) {
    case 1:
      for (auto it = ship1.begin(); it != ship1.end(); it++) {
        if (it->getCoords().first == coords.first
            && it->getCoords().second == coords.second) {
          ship1.erase(it);
          return;
        }
      }
...
void GameField::placeShipsRand() {
  ...
  for (char size = 1; size <= 4; size++) {
    for (char i = 0; i <= 4 - size; i++) {
      if (!addShip(static_cast<char>(engine() % 10),
                   static_cast<char>(engine() % 10), size)) {
      ...
      switch (size) {
        case 1:(--(ship1.end()))->updateAvailability(&cells);
          break;
        case 2:(--(ship2.end()))->updateAvailability(&cells);
          break;
        case 3:(--(ship3.end()))->updateAvailability(&cells);
          break;
        case 4:(--(ship4.end()))->updateAvailability(&cells);
          break;
        default:std::cerr << "Wrong ship size!\n";
          break;
      }
    }
  }
          ...
          if (!ship->isAlive()) {
            for (int i_ = -1; i_ <= 1; i_++) {
              for (int j_ = -1; j_ <= N; j_++) {
                if (Ship<N>::coordsValid(std::pair(coords.first + i_,
                                                   coords.second + j_ - i))) {
      ...
      case RIGHT:
        if (coords.first == ship->getCoords().first - i &&
          ...
          if (!ship->isAlive()) {
            for (int i_ = -1; i_ <= 1; i_++) {
              for (int j_ = -1; j_ <= N; j_++) {
                if (Ship<N>::coordsValid(std::pair(coords.first - j_ + i,
                                                   coords.second + i_))) {
...
std::vector<sf::Uint16> GameField::serializedField() {
  ...
  for (auto &s : ship1) {
    result.emplace_back((s.getCoords().first * 64) | (s.getCoords().second * 4)
                            | s.getDirection());
  }
  for (auto &s : ship2) {
    result.emplace_back((s.getCoords().first * 64) | (s.getCoords().second * 4)
