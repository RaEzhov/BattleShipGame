...
void Logger::log(const std::string &log, LogType type) {
    ...
    logStream <<  std::put_time(std::localtime(&t_c), "%F %T: ") << log << std::endl;
    switch (type) {
        case INFO:
            std::cout << "\033[0;37m" << logStream.str() << "\033[0m";
            break;
        case WARNING:
            std::cout << "\033[0;33m" << logStream.str() << "\033[0m";
            break;
        case ERROR:
            std::cout << "\033[0;31m" << logStream.str() << "\033[0m";
            break;
    }
